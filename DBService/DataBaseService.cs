...
using DBServices.SqlStringGenerator;
...
namespace DBServices
{
    ...
    public abstract class DataBaseService(string connectionString) : IDbService
    {
        ...
        public virtual TableBaseModel? GetRecordByKeyValue(KeyValuePair<string, object?> query, EnumQueryOperator? opertor = null, string? tableName = null)
        {
            TableBaseModel? tableResult = null;
            tableName ??= _currentTableName;
        ...
        public virtual TableBaseModel? GetRecordByForeignKey(string fieldName, long id, string? tableName = null)
        {
            tableName ??= _currentTableName;
            if (string.IsNullOrEmpty(tableName)) return null;
            TableBaseModel? tableResult = GetRecordByTableName(tableName);
            ForeignBaseModel? foreignTable = tableResult?.GetForeignKeyTable(fieldName);
            if (foreignTable == null || string.IsNullOrEmpty(foreignTable.ComeFromTableName)) return null;
            var result = GetRecordById(id, foreignTable.ComeFromTableName);
            return result;
        }
        #endregion
        #region 操作類
